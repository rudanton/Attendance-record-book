...
*   **[x] 관리자 권한**: 사장님은 외부 기기에서도 로그인하여 관리자 페이지 접근 가능.
*   **[x] 세션 안전장치**: 관리자 영역(`/admin`)을 벗어나면 자동 로그아웃 처리하여 장시간 세션 노출을 방지.
*   **[x] 클라이언트 지점 선택 저장**: 선택한 지점 ID는 로컬스토리지에 저장되어 재방문 시 자동 복원.

### 3.5 향후 개선 사항

*   **[] 웹 호스팅 배포**: Vercel 또는 Firebase Hosting을 통한 온라인 배포 (비용 0원, 외부 접근 가능, 자동으로 다중 기기 지원)
*   **[] 자동 배포 (CI/CD)**: GitHub push → 자동으로 배포 사이트 업데이트
